...
// ── Role chooser modal ────────────────────────────────────────────────────────
function RoleModal({ onClose, onStudent, onAdmin }) {
  return (
    <div style={{ position:"fixed",inset:0,zIndex:1000,background:"rgba(0,0,0,0.88)",backdropFilter:"blur(10px)",display:"flex",alignItems:"center",justifyContent:"center",padding:16 }} onClick={onClose}>
      <div style={{ background:COLORS.bgCard,border:`1px solid ${COLORS.borderLight}`,borderRadius:24,width:"100%",maxWidth:460,padding:"40px 32px",animation:"fadeIn .3s ease both" }} onClick={e=>e.stopPropagation()}>
        <button onClick={onClose} style={{ position:"absolute",top:16,right:20,background:"none",border:"none",color:COLORS.textMuted,fontSize:20,cursor:"pointer" }}>✕</button>
        <div style={{ textAlign:"center",marginBottom:32 }}>
          <div style={{ fontSize:40,marginBottom:12 }}>👋</div>
          <h2 style={{ fontFamily:"'Sora',sans-serif",fontSize:24,fontWeight:800,color:COLORS.text,marginBottom:8 }}>Who are you?</h2>
          <p style={{ color:COLORS.textMuted,fontSize:14,lineHeight:1.6 }}>Choose your role to get the right experience.</p>
        </div>
        <div style={{ display:"flex",flexDirection:"column",gap:14 }}>
          <button onClick={onStudent} style={{ background:`linear-gradient(135deg,${COLORS.accent},#a259ff)`,color:"#fff",border:"none",borderRadius:16,padding:"20px 24px",cursor:"pointer",textAlign:"left",transition:"transform 0.2s,box-shadow 0.2s",boxShadow:`0 4px 20px ${COLORS.accentGlow}` }}
            onMouseEnter={e=>{e.currentTarget.style.transform="translateY(-2px)";e.currentTarget.style.boxShadow=`0 10px 32px ${COLORS.accentGlow}`}}
            onMouseLeave={e=>{e.currentTarget.style.transform="none";e.currentTarget.style.boxShadow=`0 4px 20px ${COLORS.accentGlow}`}}>
            <div style={{ fontSize:22,marginBottom:6 }}>🎓</div>
            <div style={{ fontFamily:"'Sora',sans-serif",fontSize:17,fontWeight:800,marginBottom:4 }}>Student / Faculty / Staff</div>
            <div style={{ fontSize:13,opacity:0.85 }}>Browse events, register, track your schedule</div>
          </button>
          <button onClick={onAdmin} style={{ background:COLORS.bg,color:COLORS.text,border:`1px solid ${COLORS.borderLight}`,borderRadius:16,padding:"20px 24px",cursor:"pointer",textAlign:"left",transition:"border-color 0.2s" }}
            onMouseEnter={e=>e.currentTarget.style.borderColor=COLORS.accent}
            onMouseLeave={e=>e.currentTarget.style.borderColor=COLORS.borderLight}>
            <div style={{ fontSize:22,marginBottom:6 }}>🛡️</div>
            <div style={{ fontFamily:"'Sora',sans-serif",fontSize:17,fontWeight:800,color:COLORS.text,marginBottom:4 }}>Admin / Organizer</div>
            <div style={{ fontSize:13,color:COLORS.textMuted }}>Manage events, users, and approvals</div>
          </button>
        </div>
      </div>
    </div>
...
// ── Event preview card ────────────────────────────────────────────────────────
function PreviewCard({ ev, onClick }) {
  const free = !ev.price || ev.price === 0
  return (
    <div onClick={() => onClick(ev)} style={{ background:COLORS.bg,border:`1px solid ${COLORS.border}`,borderRadius:16,overflow:"hidden",cursor:"pointer",transition:"border-color 0.2s,transform 0.2s" }}
      ...
      {ev.image
        ? <img src={ev.image} alt={ev.name} style={{ width:"100%",height:150,objectFit:"cover",display:"block" }} />
      ...
      <div style={{ padding:"14px 16px" }}>
        ...
        <div style={{ fontFamily:"'Sora',sans-serif",fontSize:14,fontWeight:700,color:COLORS.text,marginBottom:6,lineHeight:1.3 }}>{ev.name}</div>
        <div style={{ fontSize:12,color:COLORS.textMuted,marginBottom:3 }}>📅 {ev.date}</div>
        <div style={{ fontSize:12,color:COLORS.textMuted,marginBottom:10 }}>📍 {ev.venue}</div>
        <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center" }}>
          <span style={{ fontSize:12,fontWeight:700,color:free?COLORS.free:COLORS.paid }}>{free?"Free":`ETB ${ev.price}`}</span>
          {ev.registrations>0 && <span style={{ fontSize:11,color:COLORS.textMuted }}>👥 {ev.registrations}</span>}
        </div>
      </div>
    </div>
  )
...
// ── Main export ───────────────────────────────────────────────────────────────
export default function LandingPage({ liveEvents, student, onGoTo, onGetStarted }) {
  const [showRole, setShowRole] = useState(false)
  const [visible, setVisible]   = useState(false)
  useEffect(() => { setTimeout(() => setVisible(true), 80) }, [])

  function handleGetStarted() {
    if (student) { onGoTo("events"); return }
    setShowRole(true)
  }
  function handleStudent() { setShowRole(false); onGetStarted() }
  function handleAdmin()   { setShowRole(false); window.open("http://localhost:5173", "_blank") }
  ...
  return (
    <div style={{ fontFamily:"'DM Sans',sans-serif" }}>
        ...
        <div style={{ position:"relative",maxWidth:800,opacity:visible?1:0,transform:visible?"none":"translateY(28px)",transition:"all 0.9s cubic-bezier(.16,1,.3,1)" }}>
          <div style={{ display:"inline-flex",alignItems:"center",gap:8,background:"rgba(124,92,252,0.1)",border:"1px solid rgba(124,92,252,0.25)",borderRadius:50,padding:"6px 18px",marginBottom:30 }}>
            ...
            <span style={{ fontSize:12,fontWeight:600,color:COLORS.accentLight,letterSpacing:0.6 }}>
              {liveEvents.length>0 ? `${liveEvents.length} EVENT${liveEvents.length!==1?"S":""} OPEN FOR REGISTRATION` : "AASTU OFFICIAL EVENT PLATFORM"}
            </span>
          </div>
          ...
          <div style={{ display:"flex",gap:14,justifyContent:"center",flexWrap:"wrap" }}>
            ...
            <button onClick={() => onGoTo("events")} style={{ background:"transparent",color:COLORS.text,border:`1px solid ${COLORS.borderLight}`,borderRadius:14,padding:"15px 38px",fontSize:16,fontWeight:600,cursor:"pointer",transition:"border-color 0.2s,color 0.2s" }}
      ...
      <section style={{ padding:"80px 32px",background:COLORS.bgCard,borderTop:`1px solid ${COLORS.border}`,borderBottom:`1px solid ${COLORS.border}` }}>
        <div style={{ maxWidth:1100,margin:"0 auto" }}>
          <div style={{ display:"flex",justifyContent:"space-between",alignItems:"flex-end",marginBottom:36,flexWrap:"wrap",gap:12 }}>
            <div>
              <div style={{ fontSize:11,fontWeight:700,color:COLORS.accentLight,letterSpacing:1,marginBottom:8 }}>LIVE FROM ADMIN</div>
              ...
              <p style={{ color:COLORS.textMuted,fontSize:14 }}>
                {liveEvents.length>0 ? `${liveEvents.length} approved event${liveEvents.length!==1?"s":""} — updated in real time` : "Events approved by admins appear here instantly"}
              </p>
            </div>
            <button onClick={()=>onGoTo("events")} style={{ background:"transparent",color:COLORS.accentLight,border:`1px solid ${COLORS.accent}`,borderRadius:10,padding:"10px 22px",fontSize:14,fontWeight:600,cursor:"pointer",whiteSpace:"nowrap" }}>
              See All Events →
            </button>
          </div>
          ...
          {liveEvents.length===0 ? (
            <div style={{ textAlign:"center",padding:"60px 24px",color:COLORS.textMuted }}>
              ...
              <div style={{ fontSize:13 }}>Once an admin approves an event it shows up here automatically.</div>
            </div>
          ) : (
            <div style={{ display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(260px,1fr))",gap:20 }}>
              {liveEvents.slice(0,6).map(ev=><PreviewCard key={ev.id} ev={ev} onClick={e=>{onGoTo("events",{event:e})}} />)}
            </div>
          )}
        </div>
      </section>
      ...
      <section style={{ padding:"100px 32px",textAlign:"center",position:"relative",overflow:"hidden" }}>
        ...
        <div style={{ position:"relative",maxWidth:580,margin:"0 auto" }}>
          ...
          <div style={{ display:"flex",gap:14,justifyContent:"center",flexWrap:"wrap" }}>
            ...
            <button onClick={()=>onGoTo("events")} style={{ background:"transparent",color:COLORS.text,border:`1px solid ${COLORS.borderLight}`,borderRadius:14,padding:"16px 32px",fontSize:16,fontWeight:600,cursor:"pointer" }}>
              View Events
            </button>
          </div>
          <div style={{ marginTop:24,fontSize:13,color:COLORS.textMuted }}>
            Are you an admin or organizer?{" "}
            <button onClick={handleAdmin} style={{ background:"none",border:"none",color:COLORS.accentLight,cursor:"pointer",fontWeight:600,fontSize:13 }}>
              Go to Admin Panel →
            </button>
          </div>
        </div>
      ...
      {showRole && <RoleModal onClose={()=>setShowRole(false)} onStudent={handleStudent} onAdmin={handleAdmin} />}
    </div>
  )
}
